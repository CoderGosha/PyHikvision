...
class Client:
    ...
    def getNumberPlates(self):
        ...
        response = self.req.request(
            method='post', url="http://192.168.0.221/ISAPI/Traffic/channels/1/vehicleDetect/plates",
            timeout=self.timeout, stream=True, data=payload)
        return response


def main():
    cam = Client('http://192.168.0.221', 'user', '1q2w3e4r')
